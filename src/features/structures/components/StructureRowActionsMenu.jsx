...
export const StructureRowActionsMenu = ({
  structure,
  canUpdate,
  canDelete,
  statusLoadingId,
  ...
  const loading = statusLoadingId === structure.id

  const canDeactivate = canDelete && structure.isActive
  const canActivate = canUpdate && !structure.isActive
  ...
  return (
    <>
      <IconButton
        size="small"
        aria-label="Amallar"
        aria-controls={open ? `structure-actions-${structure.id}` : undefined}
        aria-haspopup="true"
      ...
      <Menu
        id={`structure-actions-${structure.id}`}
        anchorEl={anchorEl}
        open={open}
        onClose={handleClose}
